...
<div class="bg-white shadow-md rounded-lg p-6">
    <h1 class="text-2xl font-bold mb-6">Registrar Novo Acesso</h1>
    ...
    <form method="post" action="{{ route('acessos.store') }}" class="space-y-4">
        @csrf
            ...
            <select id="visitante_id" name="visitante_id" 
                ...
                @foreach($visitantes as $visitante)
                    <option value="{{ $visitante->id }}" 
                            {{ old('visitante_id') == $visitante->id ? 'selected' : '' }}>
        ...
        <div>
            ...
            <input type="datetime-local" id="data_hora_entrada" name="data_hora_entrada" 
                   ...
                   value="{{ old('data_hora_entrada', now()->format('Y-m-d\TH:i')) }}" required>
