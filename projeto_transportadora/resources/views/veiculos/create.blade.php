...
<div class="bg-white shadow rounded p-6 max-w-2xl mx-auto">
    <h2 class="text-xl font-semibold mb-4">Novo Veículo</h2>
    ...
    <form action="{{ route('veiculos.store') }}" method="POST" class="space-y-4">
        ...
        <div>
            ...
            <select name="status_acesso" id="status_acesso" required class="mt-1 block w-full border-gray-300 rounded-md">
                <option value="ativo" {{ old('status_acesso', 'ativo') == 'ativo' ? 'selected' : '' }}>Ativo</option>
                <option value="inativo" {{ old('status_acesso') == 'inativo' ? 'selected' : '' }}>Inativo</option>
                <option value="bloqueado" {{ old('status_acesso') == 'bloqueado' ? 'selected' : '' }}>Bloqueado</option>
            </select>
        </div>
