...
<div class="bg-white shadow-md rounded-lg p-6">
    ...
    <form method="post" action="{{ route('fornecedores.update', $fornecedor->id) }}" class="space-y-4">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <input type="text" id="cnpj" name="cnpj" 
                   ...
                   value="{{ old('cnpj', $fornecedor->cnpj) }}" maxlength="14" required>
            @error('cnpj')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            <input type="text" id="responsavel" name="responsavel" 
                   ...
                   value="{{ old('responsavel', $fornecedor->responsavel) }}">
            @error('responsavel')
                <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
            @enderror
        </div>
